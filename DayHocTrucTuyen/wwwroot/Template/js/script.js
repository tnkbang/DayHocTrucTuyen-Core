...
function setUserLock(maUser, elm) {
    ...
    if ($(btnUserLock).find(">:first-child").hasClass('fa-lock')) {
...
$('#confirm-lock-user').on('click', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.tt) {
                $(btnUserLock).find(">:first-child").removeClass('fa-unlock').addClass('fa-lock')
                $(btnUserLock).attr("data-original-title", "Khóa")
                $($(btnUserLock).parent().parent().children()[5]).html('Hoạt động')
                getThongBao('success', 'Thành công', 'Mở khóa người dùng thành công !')
                thisUserLock = btnUserLock = null;
            }
            else {
                $(btnUserLock).find(">:first-child").removeClass('fa-lock').addClass('fa-unlock')
                $(btnUserLock).attr("data-original-title", "Mở khóa")
                $($(btnUserLock).parent().parent().children()[5]).html('Bị khóa')
